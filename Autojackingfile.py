...
𝐋𝐞 𝐡𝐢𝐭𝐬 𝐛𝐤𝐥~ 😋🎀
ᯓᯓᯓᯓᯓᯓᯓᯓ
𝐟𝐨𝐥𝐥𝐨𝐰𝐞𝐫𝐬 : {user_followers}
𝐅𝐨𝐥𝐥𝐨𝐰𝐢𝐧𝐠 : {user_following}
𝐏𝐨𝐬𝐭𝐬 : {total_posts}
𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞 : {username}
𝐄𝐦𝐚𝐢𝐥 : {username}@{jj}
𝐝𝐚𝐭𝐞 : {date}
...
def check(email):
  ...
  try:
   ...
   #   print(response.text)
      if 'email_is_taken' in str(response.text):Jimsrxy(email)
...
def qqq():
  while True:
    try:
      ...
      username = requests.post('https://www.instagram.com/api/graphql', headers=headers, data=data).json()['data']['user']['username']#['@Jimsrxy']
